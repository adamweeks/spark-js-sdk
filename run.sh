...
docker run -it --rm --volumes-from ${HOSTNAME} -e NPM_CONFIG_CACHE=${WORKDIR}/.npm ${DOCKER_CONTAINER_NAME} rm -rf node_modules
docker run -it --rm --volumes-from ${HOSTNAME} -e NPM_CONFIG_CACHE=${WORKDIR}/.npm ${DOCKER_CONTAINER_NAME} rm -rf packages/*/node_modules
docker run -it --rm --volumes-from ${HOSTNAME} -e NPM_CONFIG_CACHE=${WORKDIR}/.npm ${DOCKER_CONTAINER_NAME} rm -rf packages/bin-sauce-connect/.sauce
docker run -it --rm --volumes-from ${HOSTNAME} -e NPM_CONFIG_CACHE=${WORKDIR}/.npm ${DOCKER_CONTAINER_NAME} ls -la
exit 1
