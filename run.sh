...
docker run --rm --volumes-from ${HOSTNAME} --user=0:0 -e NPM_CONFIG_CACHE=${WORKDIR}/.npm ${DOCKER_CONTAINER_NAME} rm -rf node_modules
docker run --rm --volumes-from ${HOSTNAME} --user=0:0 -e NPM_CONFIG_CACHE=${WORKDIR}/.npm ${DOCKER_CONTAINER_NAME} rm -rf packages/*/node_modules
docker run --rm --volumes-from ${HOSTNAME} --user=0:0 -e NPM_CONFIG_CACHE=${WORKDIR}/.npm ${DOCKER_CONTAINER_NAME} rm -rf packages/bin-sauce-connect/.sauce
docker run --rm --volumes-from ${HOSTNAME} --user=0:0 -e NPM_CONFIG_CACHE=${WORKDIR}/.npm ${DOCKER_CONTAINER_NAME} ls -la
exit 1
