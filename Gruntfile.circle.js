...
module.exports = function gruntConfig(grunt) {
  ...
  if (CIRCLE_NODE_INDEX === CIRCLE_NODE_TOTAL - 1) {
    config.shell.legacy = {
      command: `node ./tooling/circle-scripts/legacy.js browser && npm run grunt:circle coverage`
    };
  ...
  if (CIRCLE_NODE_INDEX === CIRCLE_NODE_TOTAL - 2) {
    config.shell.legacy = {
      command: `node ./tooling/circle-scripts/legacy.js node && npm run grunt:circle coverage`
    };
