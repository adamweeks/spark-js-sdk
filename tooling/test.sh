...
rm -rf "${SDK_ROOT_DIR}/.sauce"

echo "RUNNING HTTP CORE TEST"
set -x
docker run -e PACKAGE=http-core ${DOCKER_RUN_OPTS} bash -c "npm run test:package:sauce > ${SDK_ROOT_DIR}/reports/logs/docker.http-core.log 2>&1" &
PIDS+=" $!"
set +x

echo "RUNNING MERCURY TEST"
set -x
docker run -e PACKAGE=plugin-mercury ${DOCKER_RUN_OPTS} bash -c "npm run test:package:sauce > ${SDK_ROOT_DIR}/reports/logs/docker.plugin-mercury.log 2>&1" &
PIDS+=" $!"
set +x
