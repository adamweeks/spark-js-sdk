...
export PACKAGE=common
set -x
docker run -e PACKAGE=${PACKAGE} ${DOCKER_RUN_OPTS} bash -c "pwd"
docker run -e PACKAGE=${PACKAGE} ${DOCKER_RUN_OPTS} bash -c "touch ${SDK_ROOT_DIR}/reports/proof"
docker run -e PACKAGE=${PACKAGE} ${DOCKER_RUN_OPTS} bash -c "npm run test:package:sauce > ${SDK_ROOT_DIR}/reports/logs/docker.${PACKAGE}.log 2>&1" &
PIDS+=" $!"
set +x
